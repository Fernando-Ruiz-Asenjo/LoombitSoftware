...
| 6 | Galaxia (drag-to-act) | 🟢 auditada (2026-06-10) | `resolve_drop`: combo sin regla → `_no_aplica` (seguro, sin crash); combo válido → `agent_task` enrutado al agente CON gate (no efecto sin aprobar). Cubierto por `test_galaxia_actions.py`. Sin bug. P2 e-commerce: rectificativa por devolución → factura negativa (fiscalmente OK) pero el agente narró "✅ registrada" Y "error" (contradictorio, no reproducido con fiabilidad) |
| 7 | Fábrica | ⬜ | |
| 8 | Ajustes | ⬜ | |
