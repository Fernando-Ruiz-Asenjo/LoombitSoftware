...
**★ CICLO 2/50 (2026-06-10):** batería `scripts/funcional_live3.py` (50 escenarios por ángulos AÚN más distintos: conversación MULTIVUELTA con history, decimales/separadores 1.234,56, fechas raras —ayer/fin de mes/29-feb no bisiesto—, casuística 303 —inversión sujeto pasivo, recargo, criterio caja, a devolver—, peticiones ILEGALES —factura en negro, evasión—, seguridad AVANZADA —inyección en calendario, SSRF, anti-supresión del aviso fiscal, phishing de credenciales). **BUG REAL serio destapado y arreglado:** en conversación, una respuesta de seguimiento CORTA («Emitida.», «¿y en junio?») no ruteaba (el force-tool clasifica sobre el último mensaje, no el historial) → el 14B **FABRICABA** un «✅ Factura registrada» / un número SIN llamar la tool (verificado por steps: STEPS=[task_done] o []). Fix LIMPIO: `_texto_para_intencion` en `loop.py` (si el último mensaje no tiene intención propia y es corto, hereda el último mensaje del usuario del hilo → fuerza la tool real, no puede fabricar) + foco de `factura` = {registrar_factura, ask_user} SIN task_done (no se escapa a narrar un éxito confuso). Verificado: multivuelta 5/5 EN VIVO (mt_registra ahora REGISTRA de verdad; mt_cambia_mes da el número real; mt_correo busca). +1 golden determinista. Gate verde. **Resiliencia de la herramienta:** añadido timeout por escenario (thread) a la batería → un cuelgue de API externa ya no bloquea la pasada. **Hallazgo infra detectable:** el Google Calendar READ cuelga intermitentemente (timeout de batería lo caza; las llamadas HTTP tienen timeout 15/20s, así que es slowness de LM Studio `--parallel 1` degradado o Calendar lento — NO bug de código; el read-vs-create de agenda está cubierto por v1/v2). 35 no-agenda + 5 multivuelta verde; 2 lecturas de calendario = timeout de infra. Ajustes de TEST: `mt_registra` con datos completos (la fecha faltaba → pedirla era honesto, no bug); `despide_empleado` relajado (ofrecer recordatorio no es fabricar un despido). **2º hallazgo real arreglado limpio:** ante «hazme el modelo 111/349» el 14B pedía ventas/compras (lo confundía con el 303), UX confusa → nuevo short-circuit `_modelo_no_modelado` (abstención HONESTA para los modelos AEAT no modelados 111/115/130/180/190/347/349/390; el 303 NO entra) + golden. **Pasada completa final v3: 50/50 verde** (el Calendar se recuperó; era transitorio).

**★ CICLO 3/50 (2026-06-10):** batería `scripts/funcional_live4.py` (42 escenarios) por ángulos nuevos (importes NEGATIVOS/cero/enormes, NIF inválido, conversaciones de 3-4 TURNOS, comparativas/predicciones NO soportadas → abstención honesta, vectores de seguridad nuevos) + **8 escenarios de REGRESIÓN de los fixes de ciclos 1-2** (venció/mis ventas/retenido/IBAN inválido/trimestre relativo/modelo 130/gastado/A1) — encargo de Fernando: blindar lo arreglado junto con presión nueva. 1ª pasada **41/42**; los 8 de regresión PASARON (fixes 1-2 siguen verdes) y las conversaciones de 3-4 turnos PASARON (el fix `_texto_para_intencion` aguanta hilos largos). Único fallo: `factura_devolucion_negativa` — el 14B GARBEA el importe negativo de una rectificativa al extraer args (registró base -827,96 en vez de -200, o falló honesto). **Diagnóstico: NO es bug de Loombit** — `registrar_factura(base=-200)` funciona de forma determinista y correcta (blindado en golden) (base -200 + IVA -42 = total -242); es un límite de EXTRACCIÓN del 14B en negativos. Acción (documentar + arreglar el ámbito correcto): blindada la capacidad determinista en golden `test_registrar_factura_acepta_base_negativa_rectificativa`; el check live ajustado al invariante real (rutea a registrar o falla honesto, sin fabricar). **Resultado ciclo 3/50: 42/42 verde.** Sin cambios de núcleo (el sistema era correcto). Lección: distinguir límite del MODELO (14B) de bug del SISTEMA — el primero se documenta + se blinda la parte determinista, no se «arregla» con un parche.

**★ CICLO 4/50 (2026-06-10) — AUDITORÍA DE DISEÑO (encargo Fernando: «más allá de los errores — diseño, planteamiento, carencias del proceso creativo»):** nuevo doc **`docs/AUDITORIA_DISENO_CEREBRO_2026-06-10.md`** (9 hallazgos por TIPO·SEVERIDAD·dueño). Meta-observación: ~80% de los «bugs» de los ciclos 1-3 eran de ROUTING o EXTRACCIÓN, no de cálculo → 2 problemas de raíz, no 10 sueltos. Top: **D-1** routing por regex no escala (whack-a-mole; el LLM-clasificador como router primario es el diseño correcto, ya tenemos la pieza en A1); **D-2** (deuda MÍA) dominio fiscal metido en el núcleo BLANCO `loop.py` (retención/IBAN/modelos AEAT) — viola «el dominio vive en skills»; **D-3** los NÚMEROS los extrae el 14B, no un extractor determinista (la brújula dice «cifras por código»); D-4 sin razonamiento comparativo/temporal; D-5 el telar no teje las finanzas; D-6 tests por string-matching (mejor por estructura); D-7 foco hand-tuneado; D-8/D-9 decisiones de Fernando. Batería `scripts/funcional_live5.py` (24 DESIGN-PROBES, no bugs sino carencias): **24/24** — confirma D-4/D-5 (el agente ABSTIENE honesto en comparativas/tendencias/multi-entidad/síntesis, NO fabrica) y refina D-3 (el 14B maneja números en palabras comunes —«dos mil quinientos», «1k», «un millón y medio»— y garbea solo los EDGE: negativos, total-vs-base). Hallazgo menor: artefacto `_icall_function(...)` colado al usuario cuando el 14B flaila una comparativa (atado a D-4). **Orden recomendado: D-2 (pagar mi deuda, limpio) → D-3 (extractor de importes) → D-1 (router LLM, con tu OK).**
