...
1. ✅ **Pilot verificado EN VIVO y CABLEADO (2026-06-08).** Demo (`pilot_demo.py`): cartel + cursor real (log de coordenadas exactas) + abre Google Console. **Señal visible PROPIA de Loombit** (D-18): halo de perímetro violeta→cian + halo de cursor en colores de marca (antes el halo era el naranja de Claude/Computer-Use). **Cableado al executor** (D-19): `execute_sequence` muestra el halo en todo run real; verificado e2e escribiendo 175 chars en el Bloc de notas real con recibo (`pilot_16061017.json`, `overlay_shown:true`). 🟢. *Pendiente menor: cablear el overlay/executor como tool del agente del chat (hoy se ejercita por `/loombit/pilot/execute`); y ocultar el halo durante el paso `screenshot` si llegara a estorbar la visión del agente.*
