...
## 26. Ronda 9 (30 ciclos) — fuera de la caja: la capa de soberanía personal

> 30 vueltas innovadoras (frontera). Señales reales en `docs/RADAR.jsonl` (2026-06-14, 63 señales).

### 26.1 Tanda 1 — maduración out-of-box (skills no adyacentes §25.3)
Traductor de Burocracia **+Anti-Fraude** (detecta carta/SMS falso de Hacienda/DGT) · Investigador
**adversarial** (lo que el vendedor oculta) · Ejecutor Digital +dossier pre-mortem · Legado +liberación
condicional · Onboarding +verificación · Diario +**divulgación selectiva (ZK)** · Pre-mortem +outside-view ·
Radar Renovaciones +**dark-patterns**/auto-cancelar · Memoria de Relaciones +cadencia.
**2 upgrades de kernel emergen:** (a) divulgación selectiva (probar sin revelar, eIDAS2); (b) modo
"actúa por mí" con gate (negociar/cancelar/alegar, no solo informar).

### 26.2 Tanda 2 — adyacentes innovadoras (el autónomo deja de estar solo)
`Skill D Anti-Fraude Fiscal` (phishing AEAT/DGT) · `Skill D Runway/Salud Financiera` (82% quiebran por
cash-flow; forecast 13 semanas) · `Skill A Agente-a-Agente A2A` (Agent Card sobre el MCP existente, habla
con gestoría/cliente) · `Negociador de Proveedores` · `Optimizador de Tesorería` · `Asistente de
Inspección en vivo` · `Conector TPV/e-commerce` · `Primer Empleado` (TGSS) · mejora: el ledger emite
**credenciales verificables** con divulgación selectiva (eIDAS2).

### 26.3 Tanda 3 — no adyacentes: el agente que te representa
`Defensor del Consumidor` (anti dark-patterns; FTC click-to-cancel / EU Digital Fairness Act) ·
`Negociador Personal` (factura móvil/luz/coche; CarEdge −1.500$) · `CISO Personal` · `Apoderado Digital`
(gemelo cognitivo local) · `Compañero de Duelo` (local, no terapia) · `Cápsula del Tiempo Familiar` ·
`Coach de Decisiones de Vida` · `Investigador Científico/Ciudadano` · `Director de la Casa`.

### 26.4 LA TESIS (fuera de la caja)
**LoomBit = capa de soberanía personal: el agente LOCAL que representa al usuario frente a instituciones y
empresas en un mundo digital asimétrico.** Cada empresa tiene una IA optimizada para extraer al usuario
(dark patterns, scams que suplantan a Hacienda, suscripciones-trampa); falta una IA *de su lado*, y eso
SOLO puede ser local (un agente cloud de una empresa nunca estará de tu lado). Encaje de la frontera:
A2A+MCP (habla de igual a igual) · credenciales verificables + divulgación selectiva (demuestra lo justo)
· negociador/defensor/anti-fraude (actúa por ti) · ledger con procedencia (prueba lo que pasó). **El foso
es estructural:** local-first es lo que hace posible un agente que trabaja PARA el usuario, no contra él.
La cuña fiscal es el primer terreno donde la asimetría duele; la tesis escala a cualquier persona.
Anti-dispersión (D-86): VISIÓN/radar — la cuña 1 se cierra primero.
